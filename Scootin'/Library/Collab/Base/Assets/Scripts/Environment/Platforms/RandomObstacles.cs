...
public class RandomObstacles : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        hypotenuse = (xPoint / Mathf.Cos(15)); 

        yPoint = Mathf.Sqrt(Mathf.Pow(xPoint, 2) + Mathf.Pow(hypotenuse, 2));
    ...
    public static Vector3 GetRandomPointInCollider(Bounds bounds)
    {
        ...
        return new Vector3((Random.Range(bounds.min.x, bounds.max.x) + -bounds.min.x), Random.Range(bounds.min.y, bounds.max.y), Random.Range(bounds.min.z, bounds.max.z));
